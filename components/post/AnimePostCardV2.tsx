...
/**
 * V2 responsive:
 * - Mobile (phone): card fills width (w-full) + ONLY 1 image (simple)
 * - sm+ : centered max width + show 3 images grid
 */
export default function AnimePostCardV2() {
  const post: Post = {
    id: "p_02",
    user: {
      name: "Yumi Kisaragi",
    ...
    time: "Today • 9:14 PM",
    text:
      "Quick tip: wide ‘anchor’ panel first → then cut to hands/eyes/impact frames for speed.\n\nDrop your favorite action page 👇",
    images: [
      {
  ...
  return (
    <div className="w-full sm:mx-auto sm:max-w-[680px]">
      <article
        className={cn(
      ...
      >
        ...
        <div className="pointer-events-none absolute inset-0 opacity-0 transition-opacity duration-200 group-hover:opacity-100">
          <div className="absolute -inset-24 blur-3xl opacity-35 bg-[var(--gradient-brand-soft)]" />
        ...
        {/* IMAGES
            - Mobile: show only first image (simple)
            - sm+: show 3 grid
        */}
            ...
            {post.images.map((img, idx) => (
              <div
                ...
                className={cn(
                  ...
                  idx > 0 && "hidden sm:block", // ✅ hide extra images on phone
                )}
              >
                <div className={cn("relative", idx === 0 ? "aspect-[16/10]" : "aspect-[4/3]")}>
                  <Image
                    src={img.src}
                    alt={img.alt}
                    fill
                    sizes={
                      idx === 0
                        ? "(max-width: 639px) 100vw, (max-width: 1024px) 680px, 680px"
                        : "(max-width: 1024px) 220px, 220px"
                    }
                    className={cn(
                      "object-cover",
        ...
        <section className="relative flex items-center justify-between gap-2 border-t border-divider px-4 py-3">
          ...
          <div className="flex items-center gap-1.5">
            <IconButton aria-label="Comment" variant="soft" tone="neutral" size="sm" tooltip="Comment">
              <Icon name="comment" />
            </IconButton>
            <span className="text-xs font-semibold text-foreground-muted">{compact(post.stats.comments)}</span>
          </div>
