...
export function useMobileBackClose(opts: {
  ...
  useLayoutEffect(() => {
    ...
    try {
      ...
      pushedLenRef.current = history.length; // ✅ استخدم history.length لتحديد هل يوجد entries فوقنا
    } catch {
      tokenRef.current = null;
      pushedLenRef.current = null;
    }
  ...
  useEffect(() => {
    ...
    // ✅ إذا ما في entries فوقنا (نفس length) -> pop مرة واحدة
    ...
    // ✅ في entries فوقنا -> لا تلمس history الآن، خزّن token كـ stale لتخطيه لاحقًا
